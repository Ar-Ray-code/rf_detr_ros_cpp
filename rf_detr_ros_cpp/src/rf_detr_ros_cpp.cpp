...
void RfDetrNode::onInit()
{
    ...
    this->publish_boundingbox_topic_name_ = this->get_parameter("publish_boundingbox_topic_name").as_string();
...
void RfDetrNode::colorImageCallback(const sensor_msgs::msg::Image::ConstSharedPtr &ptr)
{
    ...
    RCLCPP_INFO(this->get_logger(), "Inference time: %5ld us, Detected objects: %zu", elapsed.count(), objects.size());
...
vision_msgs::msg::Detection2DArray RfDetrNode::objects_to_detection2d(const std::vector<rf_detr_cpp::Object> &objects, const std_msgs::msg::Header &header)
{
